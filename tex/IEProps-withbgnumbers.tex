...
\maketitle
\begin{abstract}
% We propose the classification group properties according to whether they can be
% imposed on a group by assuming the subgroup lattice of the group contains an
% interval isomorphic to a given lattice.  Suppose P is a group property and there
% exists a lattice L such that if G is a group with L isomorphic to an interval
% [H,G] in Sub(G), where H is core-free in G, then G has property P.  We call such P
% core-free interval enforceable (cf-IE). Among other things we show that if a
% property and its negation could both be proved cf-IE, this would solve an important problem
...
%% If we suppose that a certain
%% lattice $L$ occurs as an upper interval $L\cong [H,G] \leq \Sub(G)$, for some
%% $1\leq H\leq G$, we would like to have a better understanding of what
%% properties of $G$ we can infer from this information. 
%% The second concern if we are given a finite lattice $L$, we would like to know whether there
%% exists a finite group $G$ for which $L\cong [H,G]$, for some $1\leq H\leq G$.
%% (As we will see below, if we could prove that such a group always exists, this
%% would settle 
%% In this case, we say that $L$ is an \defn{upper interval} in the
