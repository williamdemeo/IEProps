...
call such $\cP$ \emph{Core-free Interval Enforceable (cf-IE)}. Among other
things we show that if both a property and its negation could be proved
cf-IE, this would solve an important problem in universal algebra. 
...
\newacro{FLRP}{Finite Lattice Representation Problem}
...
$A_n\, (n\geq 4)$, and every finite nonabelian simple group.\footnote{For a
  ...
  lattice, see~\cite[Theorem 7.8.1]{Schmidt:1994}.  The proof relies on the
 \acs{CFSG} Theorem.} 
For other groups,
...
\lb H,G \rb$ for some subgroup $H$ that is core-free in $G$. %for some subgroup $H$ that is core-free in $G$.
  ...
  %% If the \acs{FLRP} has a positive answer, then there exists a finite group $G$ such
  %% that each lattice $L_i \in \sL$ is an upper interval $L_i\cong \lb H_i, G \rb \leq
...
%% what can be said about a finite group $G$ for which $L\cong \lb H,G \rb$ for some
%% $H\leq G$?  
...
%% such that $L_i \cong \lb H, G \rb$ only if $G\in \sG_i$.
...
%%   for all $L\in \sL$, $L \cong \lb H,G \rb$ for some $H\leq G$.
%% \end{enumerate}
%% \end{lemma}
\item[(D)]
...
%% with respect to satsifying $L\cong\lb H,G \rb$ for some $H\leq G$.  Such properties
...
%% with $L \cong \lb H,G \rb$ with $H$ core-free in $G$, then $G$ has property $\cP$.
%% Such \acs{cfIE} properties yield \acs{cfIE} classes of groups, and these are the
...
%% \acs{FLRP} has a positive answer if and only if every finite lattice occurs as an
%% interval in the subgroup lattice of a finite group.
%% Given a lattice $L$, 
%% the expression $L \cong \lb H, G \rb$ means ``there exist groups $H \leq G$ such that  
...
%%   $\lb H_i, G \rb$ \citep[e.g.,][Ch.~4]{alvi:1987}.}
one such
