...
\newcommand{\talkcolor}{darkred}

% \newcommand{\lb}{\ensuremath{\llbracket}}
% \newcommand{\rb}{\ensuremath{\rrbracket}}
% \newcommand{\defn}[1]{\textcolor{darkred}{\textit{\textbf{#1}}}}
 \newcommand{\defn}[1]{\textcolor{\talkcolor}{\textit{\textbf{#1}}}}
...
\date[]{\vskip5mm
{\color{\talkcolor}Iowa State University}\\[6pt]
{\small {\color{\talkcolor} Algebra and Combinatorics Seminar}\\[12pt]
April 7, 2014}
}
...
ExamplesPrezi,%
Examples,%
Example7elementPrezi,%
ASClass,%
IEProps,%
IEPropsLemma,%
IEPropsConjecture,%
IEPropsLemma2,%
SubgroupLatticeBasics,%
IntervalIsomorphisms,%
ExampleOfPermutingIso,%
%Dedekindstamp,%
%ProofOfIntervalIsomorphism,%
L7second,%
IdeaOfL7Proof,%
OSTheorem,%
OSTheoremHistory,%
...
where $H \leq K \leq G$. 
\vskip4mm
We restrict our attention to \defn{upper intervals}, where $K = G$, and ask
\vskip4mm
\begin{enumerate}
\item \emph{What intervals $\lb H, G\rb$ are possible?}
\vskip4mm
\item \emph{What properties of $G$ can be deduced from the shape of $\lb H, G\rb$?}
\end{enumerate}
\end{frame}
...
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
If $N \subnormal G$
with $N \leq H$, then  $\lb H, G \rb\cong \lb H/N, G/N \rb$, 
\vskip2mm
...
There is a remarkable theorem relating this question to the \emph{finite lattice
...
\only<2>{
\begin{center}
\includegraphics[height=30mm]{images/Palfy2}
\hskip1cm
\includegraphics[height=30mm]{images/Pudlak}
 \end{center}}
\only<3>{
\vskip5mm
If these equivalent statements turn out to be true, we say,
\emph{``the \FLRP\ has a positive answer.''}
Otherwise, \emph{``the \FLRP\ has a negative answer.''} 
\vskip2cm
}
...
\begin{center}
 ...
 \draw
    ...
    \visible<5->{\shade[top color=blue,bottom color=gray!50] 
      (U) to [out=-10,in=100] (U0) to [out=170,in=-80] (U);
      \draw (-7,0) node {$\lb U_0, U \rb:=\{V : U_0 \leq V \leq U\}$};}
    \visible<6->{  \shade[top color=blue,bottom color=gray!50] 
      (UH) to [out=-10,in=100] (H) to [out=170,in=-80] (UH);
      \draw (7,6) node {$\lb H, \<U,H\> \rb$};}
    ...
    \begin{column}{0.3\textwidth}
      \begin{center}
        \begin{tikzpicture}[scale=.3]
          \input{lattices/IntervalIsomorphisms.tex}
    \visible<1>{\shade[top color=blue,bottom color=gray!-45] 
      (U) to [out=-10,in=100] (U0) to [out=170,in=-80] (U);}
    \visible<4->{
      \fill[color=blue] 
      (U) to [out=-10,in=100] (U0) to [out=170,in=-80] (U);}
    \visible<1,4->{  \shade[top color=blue,bottom color=gray!-45] 
      (UH) to [out=-10,in=100] (H) to [out=170,in=-80] (UH);}
    % \visible<5->{\fill[color=darkred]    
    %   (H) to [out=190,in=80] (U0) to [out=10,in=-100] (H);}
    % \visible<5->{\shade[top color=darkred,bottom color=gray!0]    
    %   (UH) to [out=190,in=80] (U) to [out=10,in=-100] (UH);}
       \end{tikzpicture}
      \end{center}
    \end{column}
    \begin{column}{0.8\textwidth}
      ...
      \item<1-> If $H\subnormal \<U, H\>$, then %\\[4pt]
        ...
        \item<2-> Instead of $H\subnormal \<U,H\>$, assume only
          ...
          \begin{equation*}
            %      \label{eq:dedekind-1}
            \lb U_0, U \rb^H := \{ V\in \lb U_0, U \rb \st VH = HV\},
          \end{equation*}
        ...
        \item<3-> If $U \subnormal UH$,
          ...
          \begin{equation*}
            %  \label{eq:dedekind-2}
            \lb U_0, U \rb_H := \{ V\in \lb U_0, U \rb \st H\leq N_{UH}(V)\}, 
          \end{equation*}
...
\begin{frame}[fragile,label=Dedekindstamp]{Proof of the Interval Isomorphism Lemma}
%\vskip-1cm
%The proof relies on Dedekind's modular law for subgroups.
\vskip3mm
\begin{theorem}[Dedekind's Modular Law for Subgroups]
      ...
      \begin{center}
        {\scalefont{.75}
          \begin{tikzpicture}[scale=.35]
            ...
            \fill[color=blue] 
            (U) to [out=-10,in=100] (U0) to [out=170,in=-80] (U);
            \shade[top color=blue,bottom color=gray!-45] 
            (UH) to [out=-10,in=100] (H) to [out=170,in=-80] (UH);
...
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
Let $\G$ denote the class of all finite groups.
\vskip4mm
...
\text{ if $\;G\in \G$ and $L\cong \lb H, G \rb$ and $H$ core-free, then $G$ has property $\cP$.}
\]
\vskip2mm
...
\begin{frame}[fragile,label=L7second,shrink=5]{}
  \begin{center}
    {\scalefont{.75}
      ...
      \draw (-1, 2.5) node {\color{blue}{{\large $L$}}};
...
Suppose $H<G$, $\core_G(H) = 1$, and 
 {\large {\color{blue}{$L$}}} $\cong \lb H, G \rb$.  Then
\begin{enumerate}[(i)]
...
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
\emph{But this strategy surely fails!}
\note{As the next result shows, if a group property and its
...
\begin{proof}
Assume both $\cP$ and $\neg \cP$
are IE by group representable lattices $L$ and $L_c$. 
\vskip2mm
...
\begin{frame}[label=IEPropsConjecture]{Example}
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
Insolubility is interval enforceable, but solubility is not. 
\vskip4mm
For if $L\cong \lb H, G \rb$, then for any insoluble 
group $K$ we have $L\cong \lb H\times K, G\times K \rb$, and $G\times K$ is
insoluble.  
\vskip4mm
\note{The group $H\times K$ at the bottom of
...
\vskip4mm
  ...
  \cong \lb H, G \rb$. 
\vskip4mm
 Apply the idea of Hans Kurzweil twice:
  \begin{itemize}
  \item Fix a finite nonabelian simple group $S$. 
\vskip2mm
\item Suppose the index of $H$ in $G$ is $|G:H| = n$.
\vskip2mm
\item The action of $G$ on cosets of $H$ induces an automorphism of
$S^n$ by permutation of coordinates.  
\vskip2mm
\item
Denote this by  $\phi: G \rightarrow \Aut(S^n)$, 
  and let $\phi(G) = \bar{G} \leq \Aut(S^n)$.  
...
\begin{center}
  ...
  \begin{tikzpicture}[scale=.65]
    ...
    \draw (SnG) node [above] {$S^n \bar{G}= S\wr G$};
   \draw
    ...
    \draw 
    (-3.75,3.75) node {$\prod_n^*$}
    (-.75,6.75) node {$L^*$}
    (2.3,2.25) node {$L$};
  \end{tikzpicture}
}\end{center}
The interval  $\lb D, S^n \rb$ is isomorphic to $\prod_n^*$, the dual of the lattice of
partitions of an $n$-element set.
\vskip4mm
  The dual lattice $L^*$ is an upper interval of $\Sub(S\wr G)$, namely,
  $L^*\cong \lb D\bar{G}, S\wr G \rb$. 
\vskip4mm
Repeat to get $L = L^{**}$ as an upper interval, and then check core-free!
\end{frame}
...
not include wreath products of the form $S\wr U$, where $S$ is an arbitrary
finite nonabelian simple group, is not a core-free interval enforceable class. 

\vskip4mm
\uncover<2->{
\alert{Examples:} soluble groups, simple groups, almost simple groups.}
\end{frame}
...
\]
\item[(D)]
For any finite collection $\sL$ of finite lattices, there exists a single finite
...
\begin{frame}[fragile,label=ParachutesSideBySide]{}
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
\begin{center}
{\scalefont{.8}
...
\begin{frame}[fragile,label=ParachutesNoLabels]{}
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
\begin{center}
{\scalefont{.8}
\begin{tikzpicture}[scale=0.7]
...
\vskip6mm
By (D), we consider finite collections of finite lattices and ask
what can be proved about $G$ if we assume that all of these lattices are
...
\begin{frame}[fragile,label=OSTheoremCameron]{Aschbacher-O'Nan-Scott Theorem}
